...
class PartenairesController extends AbstractController
{
    ...
    public function register(Request $request, UserPasswordEncoderInterface $passwordEncoder, EntityManagerInterface $entityManager, UserRepository $user, SerializerInterface $serializer, ValidatorInterface $validator)
    {
        ...
        } elseif ($profil == "superadminparte") {
            $role = ["ROLE_SUPER_ADMINPARTE"];
        } elseif ($profil == "user") {
            $role = ["ROLE_USER"];
        }
        $user->setRoles($role);
    ...
    /** 
     * @Route("/transac", name="list_transaction", methods={"GET", "POST"})
     */

    public function index1(Request $request, TransactionRepository $trans, SerializerInterface $serializer)
    {
